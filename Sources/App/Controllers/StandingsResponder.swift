...
final class StandingsResponder {
    ...
    func respond(_ request: Request) throws -> ResponseRepresentable {
        ...
        return try Response.async({ (portal) in
            self.dm.getStandings(seasonId: seasonId, federationId: federationId, leagueId: leagueId, roundId: roundId, callback: { (results) in
